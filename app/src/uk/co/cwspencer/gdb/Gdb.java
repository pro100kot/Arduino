...
public class Gdb
{
	...
	//!!!! private
	public final Map<String, GdbVariableObject> m_variableObjectsByExpression =
		new HashMap<String, GdbVariableObject>();
	//!!!!! private
	public final Map<String, GdbVariableObject> m_variableObjectsByName =
		new HashMap<String, GdbVariableObject>();
	...
	public synchronized boolean hasCapability(String capability)
	{
		...
		{
			System.out.println("LOG: Capabilities list is null; returning 'unsupported' for capability " +
				capability);
			return false;
		}
	...
	private void runGdb(String gdbPath, String workingDirectory)
	{
		try
		{
			...
			while ((bytes = stream.read(buffer)) != -1)
			{
				...
				{
					System.out.println("LOG: GDB/MI parsing error. Current buffer contents: \"" +
		...
		final GdbEventCallback callback)
	{
		...
		if (!(event instanceof GdbVariables))
		{
			...
			System.out.println("LOG: Unexpected event " + event + " received from -stack-list-variables " +
				"request");
			return;
		}
		...
		GdbEventCallback callback)
	{
		...
		if (!(event instanceof GdbVariableObject))
		{
			...
			errorEvent.message = "Unexpected data received from GDB";
			callback.onGdbCommandCompleted(errorEvent);
			System.out.println("LOG: Unexpected event " + event + " received from -var-create request");
			return;
		}
		...
		if (variableObject.name == null)
		{
			...
			errorEvent.message = "Unexpected data received from GDB";
			callback.onGdbCommandCompleted(errorEvent);
			System.out.println("LOG: Variable object returned by GDB does not have a name");
			return;
		}
		...
		GdbEventCallback callback)
	{
		...
		if (changes.changes != null)
		{
			for (GdbVariableObjectChange change : changes.changes)
			{
				if (change.name == null)
				{
					System.out.println("LOG: Received a GDB variable object change with no name");
					continue;
				}
				...
				if (variableObject == null)
				{
					System.out.println("LOG: Received a GDB variable object change for a variable object " +
						"that does not exist");
					continue;
				}
	...
	private void onGdbCapabilitiesReady(GdbEvent event)
	{
		if (event instanceof GdbErrorEvent)
		{
			System.out.println("LOG: Failed to get GDB capabilities list: " + ((GdbErrorEvent) event).message);
			return;
		}
		if (!(event instanceof GdbFeatures))
		{
			System.out.println("LOG: Unexpected event " + event + " received from -list-features request");
			return;
		}
