...
public class GdbMiMessageConverter
{
	...
	public static GdbEvent processRecord(GdbMiResultRecord record, String commandType)
	{
		...
		for (Class<?> clazz : GdbMiEventTypes.classes)
		{
			...
			{
				System.out.println("LOG: Class " + clazz.getName() + " is in the GdbMiEventTypes.classes list " +
				...
				if (match)
				{
					...
					if (commandType != null && clazz.equals(GdbDoneEvent.class))
					{
						for (Class<?> doneEventClass : GdbMiEventTypes.doneEventTypes)
						{
							...
							{
								System.out.println("LOG: Class " + doneEventClass.getName() + " is in the " +
							...
							if (doneEventAnnotation.command().equals(commandType))
							{
								...
								if (!doneEventAnnotation.transpose().isEmpty())
								{
									...
									{
										System.out.println("LOG: Class " + doneEventClass.getName() + " is " +
											"trying to transpose '" +
	...
	public static Object processObject(Class<?> clazz, List<GdbMiResult> results)
	{
		...
		{
			System.out.println("LOG: Failed to convert GDB/MI message to a Java object" + ex);
			return null;
		}
	}
		...
		IllegalAccessException
	{
		...
		if (!foundValueType)
		{
			System.out.println("LOG: Annotation on " + field.getName() + " requires on of GDB/MI types " +
		...
		IllegalAccessException
	{
		...
		{
			System.out.println("LOG: Annotation on " + field.getName() + " has value processor " +
		...
		{
			System.out.println("LOG: Field to invoke value processor for field " + field.getName() + " with " +
		...
		{
			System.out.println("LOG: Field " + field.getName() + " is of type " + field.getType() + ", but " +
		...
		{
			System.out.println("LOG: Failed to set value on field " + field + ex);
			return;
		}
	}
		...
		InvocationTargetException, IllegalAccessException
	{
		...
		{
			System.out.println("LOG: No conversion rules were available to convert GDB/MI result '" +
